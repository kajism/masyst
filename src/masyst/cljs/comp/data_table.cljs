(ns masyst.cljs.comp.data-table
...
(defn show-date [value]
  (let [s (cljc.util/datetime-to-str value)]
   ...
   (let [state (re-frame/subscribe [:table-state table-id])
         ...
         sort-fns (ratom/reaction
                   (let [sort-key-fn (when @sort-key-fn*
                                       (cond->> @sort-key-fn*
                                         (tran/bigdec? (some-> @rows first (@sort-key-fn*)))
                                         (comp cljc.util/parse-float #(when % (.-rep %)))
...
(defn td-comp [value last?]
  [:td {:class (str #_"text-nowrap"
                    (when last? " buttons")
                    (when (or (number? value) (tran/bigdec? value)) " text-right"))}
   (cond
     (or (string? value) (vector? value))
     value
     (= js/Date (type value))
     (show-date value)
     (number? value)
     (util/money->text value)
     (tran/bigdec? value)
     (util/money->text (cljc.util/parse-int (.-rep value)))
     (= (type value) js/Boolean)
     (util/boolean->text value)
     :else
     (str value))])
...
(defn data-table [& {:keys [table-id order-by desc? rows-per-page row-checkboxes? rows colls sum-format] :as args :or {sum-format "%d"}}]
                  ...
                  [:div.suma [:span {:dangerously-set-inner-HTML {:__html "&Sigma; "}}]
                   (util/money->text
                    (->> (:filtered-rows @table-rows)
                         (keep f)
                         (map #(if (tran/bigdec? %)
                                 (cljc.util/parse-float (.-rep %))
                                 %))
                         (apply +)
                         (gstring/format sum-format))) #_" Kč" [:br]])
           ...
           [:ul.pager
            ...
            [:li.next
             [:a {:class (str "" (when (= (:row-to @table-rows) (count (:filtered-rows @table-rows))) "btn disabled"))
              ...
              "Následující"]]])
         (when (= table-id :handovers)
           [re-com/button :label "Export" :on-click #(re-frame/dispatch [:masyst.cljs.ebs-handover/export (get @search-colls 1) (map :db/id  (:final-rows @table-rows))])])
         ]))))
